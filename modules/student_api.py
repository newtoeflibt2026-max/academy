...
from modules.audio_logic import save_audio_blob, evaluate_speaking, evaluate_writing
import os, time
...
def next_question():
    ...
    row = query_db(
        "SELECT * FROM questions WHERE skill_id=? AND is_active=1 ORDER BY RANDOM() LIMIT 1",
        (skill_id,), one=True
    )
    if not row:
        return jsonify({"question_text": None, "message": "No questions available"})
...
def leaderboard():
    rows = query_db("SELECT telegram_id, name, xp, level FROM students WHERE placement_done=1 ORDER BY xp DESC LIMIT 10")
    return jsonify([dict(r) for r in rows])
...
def add_error():
    data = request.get_json()
    execute_db(
        "INSERT INTO error_bank (student_id, question_id, skill_type) VALUES (?,?,?)",
