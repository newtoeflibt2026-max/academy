...
# ── استيراد الإعدادات ────────────────────────────────────────
try:
    from config import ADMIN_IDS, DATABASE_PATH, WEBAPP_PORT, BOT_TOKEN
except ImportError:
...
# ════════════════════════════════════════════════════════════
#  🏠 الصفحات الرئيسية — كل المسارات مسجّلة هنا
# ════════════════════════════════════════════════════════════
@app.route("/")
def home():
    """الصفحة الرئيسية — توجيه إلى Dashboard"""
    return redirect("/dashboard")

@app.route("/health")
def health_check():
    """نقطة فحص الصحة — للتأكد أن السيرفر يعمل"""
    return jsonify({
        "status": "online",
        "app": "يامن أكاديمي",
        "admin_ids": ADMIN_IDS,
        "timestamp": str(datetime.now()),
        "routes": [str(r) for r in app.url_map.iter_rules() if not r.rule.startswith('/static')]
    })

@app.route("/dashboard")
@app.route("/dashboard/<int:student_id>")
def dashboard(student_id=None):
    """صفحة الطالب Pro Light"""
    ...
    try:
        ...
        return render_template("dashboard.html",
            student=student, courses=[dict(r) for r in courses],
            error_count=err["cnt"] if err else 0, leaderboard=[dict(r) for r in lb])
    except Exception as e:
...
@app.route("/admin")
def admin_panel():
    """👑 لوحة الإمبراطورة — يجب أن تكون متاحة دائماً"""
    print(f"🟢 /admin route hit at {datetime.now()}")
    try:
        return render_template("admin.html")
    except Exception as e:
        print(f"❌ Admin render error: {e}")
        return f"<h1>⚠️ خطأ في تحميل لوحة الإدارة</h1><p>{e}</p>", 500
...
def api_toggle_skill():
    ...
    if skill_type not in SKILLS_MAP:
        return jsonify({"error":"مهارة غير معروفة"}),400
...
# ════════════════════════════════════════════════════════════
#  📡 API — الدروس + الأسئلة + XP + الأخطاء + الإحصائيات
# ════════════════════════════════════════════════════════════
...
def api_add_lesson():
    ...
    execute_db("INSERT INTO lessons (title,type,url,skill_type,is_active) VALUES (?,?,?,?,1)",(title,lesson_type,url,skill_type))
...
def api_next_question():
    ...
    if not q: return jsonify({"error":"لا توجد أسئلة لهذه المهارة"}),404
    if student_id: execute_db("UPDATE students SET last_active=datetime('now') WHERE telegram_id=?",(student_id,))
    return jsonify({"question":dict(q),"has_timer":True,"time_limit":dict(q).get("time_limit",45)})
...
with app.app_context():
    ...
    print(f"🔑 BOT_TOKEN: {'✅ Set' if BOT_TOKEN and BOT_TOKEN != 'YOUR_BOT_TOKEN_HERE' else '⚠️ NOT SET'}")
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting Yamen Academy on 0.0.0.0:{WEBAPP_PORT}")
