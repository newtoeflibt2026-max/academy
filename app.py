...
# Temporary migration endpoint (REMOVE AFTER USE)
# ════════════════════════════════════════════════
@app.route("/admin/run-packages-migration")
def _run_packages_migration():
    """Apply the 8-packages migration to the production DB. Token-protected."""
    ...
    try:
        ...
        cur.execute("UPDATE subscription_plans SET is_active=0")
        log.append(f"deactivated {cur.rowcount} old plans")

        # 2) add new columns to subscription_plans
        new_plan_cols = {
            "section_code": "TEXT DEFAULT ''",
            "writing_review_quota": "INTEGER DEFAULT 0",
            "speaking_review_quota": "INTEGER DEFAULT 0",
            "requires_promo_task": "INTEGER DEFAULT 0",
            "sort_order": "INTEGER DEFAULT 0",
        }
        existing = [r[1] for r in cur.execute("PRAGMA table_info(subscription_plans)").fetchall()]
        for col, ddl in new_plan_cols.items():
            if col not in existing:
                cur.execute(f"ALTER TABLE subscription_plans ADD COLUMN {col} {ddl}")
                log.append(f"+col plans.{col}")

        # 3) insert/update 8 plans
        plans = [
            ("free_15d","🆓 الباقة المجانية (15 يوم)",0.0,"JOD",15,"free",0,0,1,1,
                "درس واحد يومياً (قراءة أو تأسيس) لمدة 15 يوم — مرة واحدة فقط — بشرط مهمة دعائية وموافقة الإدارة.",
                "✅ درس يومي\n✅ قراءة أو تأسيس\n⚠️ مهمة دعائية\n⚠️ مرة واحدة فقط"),
            ("reading_45d","📖 باقة القراءة",50.0,"JOD",45,"reading",0,0,0,2,
                "كل دروس وامتحانات قسم القراءة لمدة 45 يوم.",
                "✅ كل دروس القراءة\n✅ كل التدريبات\n✅ امتحان نهاية القسم\n✅ بنك الأخطاء"),
            ("listening_45d","🎧 باقة الاستماع",50.0,"JOD",45,"listening",0,0,0,3,
                "كل دروس وامتحانات قسم الاستماع لمدة 45 يوم.",
                "✅ كل دروس الاستماع\n✅ كل التدريبات\n✅ امتحان نهاية القسم\n✅ بنك الأخطاء"),
            ("writing_45d","✍️ باقة الكتابة",50.0,"JOD",45,"writing",2,0,0,4,
                "كل دروس الكتابة + تصحيحان من الأستاذ لمدة 45 يوم.",
                "✅ كل دروس الكتابة\n✅ تصحيح بشري × 2\n✅ امتحان نهاية القسم"),
            ("speaking_45d","🗣️ باقة المحادثة",50.0,"JOD",45,"speaking",0,2,0,5,
                "كل دروس المحادثة + تصحيحان من الأستاذ لمدة 45 يوم.",
                "✅ كل دروس المحادثة\n✅ تصحيح بشري × 2\n✅ امتحان نهاية القسم"),
            ("foundation_45d","🏗️ باقة التأسيس",50.0,"JOD",45,"foundation",0,0,0,6,
                "التأسيس الشامل (المراحل الأربع F1-F4) لمدة 45 يوم.",
                "✅ كل دروس التأسيس\n✅ بوابات المراحل\n✅ بنك الأخطاء"),
            ("mock_15d","📝 باقة الامتحانات التجريبية",50.0,"JOD",15,"mock",2,2,0,7,
                "3-5 امتحانات TOEFL تجريبية + 2 كتابة + 2 محادثة تصحيح لمدة 15 يوم.",
                "✅ 3-5 امتحانات تجريبية\n✅ تصحيح كتابة × 2\n✅ تصحيح محادثة × 2"),
            ("full_90d","👑 الباقة الشاملة",200.0,"JOD",90,"full",2,2,0,8,
                "كل الأقسام + التأسيس + الامتحانات + تصحيح. توفير 150 JOD.",
                "✅ كل الأقسام\n✅ التأسيس\n✅ الامتحانات التجريبية\n✅ تصحيح كتابة × 2\n✅ تصحيح محادثة × 2"),
        ]
        for name,name_ar,price,curr,days,sect,wq,sq,promo,sort,desc,feats in plans:
            row = cur.execute("SELECT id FROM subscription_plans WHERE name=?",(name,)).fetchone()
            if row:
                cur.execute("""UPDATE subscription_plans SET name_ar=?,price=?,currency=?,duration_days=?,
                    description=?,features=?,is_active=1,is_featured=?,section_code=?,writing_review_quota=?,
                    speaking_review_quota=?,requires_promo_task=?,sort_order=? WHERE name=?""",
                    (name_ar,price,curr,days,desc,feats,1 if name=="full_90d" else 0,sect,wq,sq,promo,sort,name))
                log.append(f"updated {name}")
            else:
                cur.execute("""INSERT INTO subscription_plans (name,name_ar,price,currency,duration_days,
                    description,features,is_active,is_featured,created_at,section_code,writing_review_quota,
                    speaking_review_quota,requires_promo_task,sort_order) VALUES (?,?,?,?,?,?,?,1,?,?,?,?,?,?,?)""",
                    (name,name_ar,price,curr,days,desc,feats,1 if name=="full_90d" else 0,
                     datetime.datetime.now().isoformat(timespec="seconds"),sect,wq,sq,promo,sort))
                log.append(f"inserted {name}")

        # 4) add student columns
        new_st_cols = {
            "subscription_section": "TEXT DEFAULT ''",
            "writing_review_remaining": "INTEGER DEFAULT 0",
            "speaking_review_remaining": "INTEGER DEFAULT 0",
            "free_plan_review_requested_at": "TEXT",
            "promo_task_status": "TEXT DEFAULT ''",
            "promo_task_proof": "TEXT",
        }
        existing = [r[1] for r in cur.execute("PRAGMA table_info(students)").fetchall()]
        for col, ddl in new_st_cols.items():
            if col not in existing:
                cur.execute(f"ALTER TABLE students ADD COLUMN {col} {ddl}")
                log.append(f"+col students.{col}")

        # 5) section_pass_scores
        cur.execute("""CREATE TABLE IF NOT EXISTS section_pass_scores (
            section TEXT PRIMARY KEY, max_score INTEGER NOT NULL,
            pass_score INTEGER NOT NULL, note TEXT)""")
        for s,mx,ps,note in [("reading",30,18,"target≥59"),("listening",30,18,"target≥59"),
                             ("writing",30,22,"AI+human"),("speaking",30,22,"AI+human"),
                             ("foundation",100,80,"إتقان")]:
            cur.execute("""INSERT INTO section_pass_scores (section,max_score,pass_score,note)
                VALUES (?,?,?,?) ON CONFLICT(section) DO UPDATE SET
                max_score=excluded.max_score,pass_score=excluded.pass_score,note=excluded.note""",
                (s,mx,ps,note))

        con.commit()

        # final report
        active_plans = cur.execute("""SELECT name,name_ar,price,duration_days,section_code
            FROM subscription_plans WHERE is_active=1 ORDER BY sort_order""").fetchall()
        con.close()

        return jsonify({
            "ok": True,
            "log": log,
            "active_plans": [{"name":r[0],"name_ar":r[1],"price":r[2],"days":r[3],"section":r[4]} for r in active_plans],
