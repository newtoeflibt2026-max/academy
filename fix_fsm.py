...
def extract_file_id(msg):
    ...
    if msg.audio:
        ...
        mt, fid = 'video_note', msg.video_note.file_id
    elif msg.photo:
        ...
        mt, fid = 'sticker', msg.sticker.file_id
    if fid:
        print(f"DEBUG: media={mt} file_id={fid[:30]}...")
    else:
        print(f"DEBUG: no media detected. content_type={msg.content_type}")
    return mt, fid
...
async def add_lesson_level(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("Media? Upload file or press Skip (/cancel to abort)", reply_markup=kb); await cb.answer()
...
async def media_type_selected(cb: types.CallbackQuery, state: FSMContext):
    mt = cb.data.split("_")[1]
    await state.update_data(media_type=mt)
    await cb.message.edit_text(f"Now UPLOAD ({mt}) file from your device. /cancel to abort")
    await cb.answer()

# ONE handler for ALL message types during media state
...
async def receive_media(msg: types.Message, state: FSMContext):
    ...
    if fid:
        ...
        await msg.answer(
            "No media detected.\\n"
            "Upload a file FROM YOUR DEVICE (not forward).\\n"
            "Or press Skip. /cancel to abort."
        )
...
async def add_place_mcq(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("question|opt1|opt2|opt3|opt4|correct|level (/cancel to abort)"); await cb.answer()
...
async def add_place_spell(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("word|level|hint (/cancel to abort)"); await cb.answer()
...
async def add_quiz_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("Pick lesson: (/cancel to abort)", reply_markup=kb); await cb.answer()
...
async def import_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("Send file (/cancel to abort)"); await cb.answer()
...
async def chg_group_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("New link: (/cancel to abort)"); await cb.answer()
...
print("- Added /cancel command for ALL FSM states")
print("- extract_file_id with debug logging")
print("- Clear instructions: UPLOAD from device, not forward")
