...
DB_PATH = os.path.join(BASE, "data", "academy.db")

def update_database():
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    try: cur.execute("ALTER TABLE placement_questions ADD COLUMN question_type TEXT DEFAULT 'mcq'")
    except: pass
    try: cur.execute("ALTER TABLE quiz_questions ADD COLUMN question_type TEXT DEFAULT 'mcq'")
    except: pass
    try: cur.execute("ALTER TABLE quiz_questions ADD COLUMN options TEXT DEFAULT ''")
    except: pass
    try: cur.execute("ALTER TABLE quiz_questions ADD COLUMN correct_answer TEXT DEFAULT ''")
    except: pass
    cur.execute("CREATE TABLE IF NOT EXISTS group_settings (key TEXT PRIMARY KEY, value TEXT)")
    cur.execute("INSERT OR IGNORE INTO group_settings (key, value) VALUES ('discussion_group', 'https://t.me/+2NkF901AApcyODk0')")
    try: cur.execute("ALTER TABLE students ADD COLUMN phone TEXT DEFAULT ''")
    except: pass
    conn.commit()
    conn.close()
...
# --- subscriptions.py ---
subs_code = """from aiogram import Router, F, types
...
subs_path = os.path.join(BASE, "handlers", "subscriptions.py")
with open(subs_path, "w", encoding="utf-8") as f:
    f.write(subs_code)
print("subscriptions.py done")
...
async def add_lesson_level(cb: types.CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Video", callback_data="media_video"), InlineKeyboardButton(text="Skip", callback_data="media_skip")],
    ])
...
async def receive_media(msg: types.Message, state: FSMContext):
    d = await state.get_data(); mt = d.get('media_type'); fid = None
...
async def add_quiz_lesson(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(f"Quiz #{qid} created. Send questions (/done to finish)"); await cb.answer()
...
async def import_file(msg: types.Message, state: FSMContext):
    ...
    if itype == "json":
        ...
        for row in csv.reader(io.StringIO(content)):
            if row: add_spelling_word(row[0], row[1] if len(row)>1 else "", row[2] if len(row)>2 else "A1"); count += 1
    elif itype == "txt":
...
async def list_lessons_handler(cb: types.CallbackQuery):
    lessons = get_all_lessons()
    text = "Lessons:\\n" + "\\n".join([f"#{l['id']} {l['title']}" for l in lessons[:30]])
    await cb.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def view_lesson(cb: types.CallbackQuery):
    ...
    if lesson.get('media_type') == 'photo' and lesson.get('media_file_id'):
        await cb.message.answer_photo(lesson['media_file_id'], caption=text, reply_markup=kb)
        await cb.message.delete()
    elif lesson.get('media_type') == 'audio' and lesson.get('media_file_id'):
        await cb.message.answer_audio(lesson['media_file_id'], caption=text, reply_markup=kb)
        await cb.message.delete()
    elif lesson.get('media_type') == 'video' and lesson.get('media_file_id'):
        await cb.message.answer_video(lesson['media_file_id'], caption=text, reply_markup=kb)
        await cb.message.delete()
    else:
